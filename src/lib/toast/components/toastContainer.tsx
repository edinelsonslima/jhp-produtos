...
import type { IToast, IToastContainerProps, IToastData } from '../@types'

export function ToastContainer({ defaultDuration, children, ...props }: IToastContainerProps) {
  ...
  useEffect(() => {
    function handleAddToast(data: IToast) {
      ...
      setMessages((prevMessages) => [...prevMessages, { id: generateUUID(), content, duration, type }])
    }
  ...
  return createPortal(
    <div {...props}>{messages.map((message) => children({ ...message, remove: handleRemoveToastMessage }))}</div>,
    document.body,
  )
}
